...
wp core install --url=$DOMAIN_NAME/ --title=$WORDPRESS_TITLE --admin_user=$WORDPRESS_ADMIN_USR --admin_password=$WORDPRESS_ADMIN_PWD --admin_email=$WORDPRESS_ADMIN_EMAIL --skip-email --allow-root
...
wp user create $WORDPRESS_USR $WORDPRESS_EMAIL --role=author --user_pass=$WORDPRESS_PWD --allow-root
